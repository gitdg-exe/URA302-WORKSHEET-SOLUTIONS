...
for i in range(len(path)-1):
    c.create_line(path[i],path[i+1],fill="blue",width=3)
...
def up(): c.move(robot,0,-10)
def down(): c.move(robot,0,10)
def left(): c.move(robot,-10,0)
def right(): c.move(robot,10,0)
...
c.create_line(A,270,fill="red")
c.create_line(270,270,fill="green")
c.create_line(270,300,fill="blue")
...
def key(e):
    x,y = 0,0
    if e.keysym=="Up": y=-5
    if e.keysym=="Down": y=5
    if e.keysym=="Left": x=-5
    if e.keysym=="Right": x=5
    c.move(path_robot,x,y)
    trail.append(c.create_line(c.coords(path_robot),fill="black"))
